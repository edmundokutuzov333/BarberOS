            ...
            <div key={h.id} data-testid={`haircut-card-${h.id}`} className={`glass glass-3 !rounded-2xl overflow-hidden ${!h.is_active && 'opacity-50'}`} style={{ boxShadow: 'none' }}>
              <div className="aspect-[3/4] bg-[var(--surface-2)] relative">
                ...
                <div className="absolute top-2 right-2 flex gap-1">
                  ...
                  <button type="button" data-testid={`haircut-edit-${h.id}`} onClick={() => open(h)} aria-label="Editar" className="h-8 w-8 rounded-full bg-black/50 backdrop-blur grid place-items-center text-ink-hi hover:text-accent-soft"><Pencil size={13} /></button>
                  <button type="button" data-testid={`haircut-delete-${h.id}`} onClick={() => confirm(`Remover "${h.name}"?`) && remove.mutate(h)} aria-label="Remover" className="h-8 w-8 rounded-full bg-black/50 backdrop-blur grid place-items-center text-ink-hi hover:text-st-noshow"><Trash2 size={13} /></button>
                </div>
              </div>
