...
export default function AdminPlanos(){
  ...
  const save=async()=>{if(!selected)return;await mutation.mutateAsync({id:selected.id,name:selected.name,priceCents:selected.price_cents,maxBarbers:selected.max_barbers,features:(selected.features||{}) as Record<string, Json>,isActive:selected.is_active});setSelected(null);};
    ...
    <div className="grid xl:grid-cols-[1fr_420px] gap-3">
      ...
      <Panel title={selected?'Editar '+selected.name:'Seleccione um plano'}>
        {selected?<div className="space-y-4">
          ...
          <div className="space-y-2"><p className="t-label text-ink-mid">Funcionalidades</p>{['deposits','whatsapp','multi_location'].map(k=><FeatureToggle key={k} label={k==='deposits'?'Sinais':k==='whatsapp'?'WhatsApp':'Multi-localização'} checked={Boolean((selected.features as Record<string,unknown>)?.[k])} onChange={value=>setSelected({...selected,features:{...(selected.features as Record<string, Json>),[k]:value} as Json})}/>)}</div>
