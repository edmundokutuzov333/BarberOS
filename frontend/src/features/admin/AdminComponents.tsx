...
export function AdminMetricCard({ label, value, detail, icon: Icon }: {
  label: string; value: ReactNode; detail?: string; icon: ComponentType<{size?: number; className?: string}>;
}) {
  return (
...
export function AdminStatus({ kind, value }: { kind:'shop'|'payment'|'support'|'priority'; value:string }) {
  const label = kind==='shop' ? SHOP_STATUS[value] : kind==='payment' ? PAYMENT_STATUS[value] : kind==='support' ? SUPPORT_STATUS[value] : PRIORITY[value];
  return <span className={cn('inline-flex rounded-full border px-2.5 py-1 t-label', kind==='priority'&&value==='urgent'&&'border-st-noshow/40 text-st-noshow', kind==='priority'&&value==='high'&&'border-st-warning/40 text-st-warning')}>{label ?? value}</span>;
}
...
export function Pager({ offset, pageSize, total, onPrev, onNext }: { offset:number; pageSize:number; total:number; onPrev:()=>void; onNext:()=>void }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 mt-4">
      <p className="t-label text-ink-lo">{total ? 'Página ' + current + ' de ' + pages : 'Sem registos'}</p>
      <div className="flex items-center gap-2">
        <Button variant="secondary" size="sm" aria-label="Página anterior" onClick={onPrev} disabled={offset===0}><ChevronLeft size={14}/></Button>
        <Button variant="secondary" size="sm" aria-label="Página seguinte" onClick={onNext} disabled={offset+pageSize>=total}><ChevronRight size={14}/></Button>
      </div>
    </div>
  );
}
...
export function FeatureToggle({ label, checked, onChange }: { label:string; checked:boolean; onChange:(value:boolean)=>void }) {
  return <label className="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/5 px-3.5 py-3 cursor-pointer"><span className="text-sm text-ink-hi">{label}</span><input type="checkbox" checked={checked} onChange={e=>onChange(e.target.checked)} className="h-4 w-4 accent-[var(--accent)]" /></label>;
}
