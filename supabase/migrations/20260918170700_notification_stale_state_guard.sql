-- BarberOS Phase 15 patch: claim only notifications whose domain state is still current.
...
begin
  with due as (
    ...
    where n.status='queued'
      ...
      and (
        ...
        or (
          n.template_key='appointment_pending'
          and a.status='pending'
        )
        or (
          n.template_key='appointment_confirmed'
          and a.status='confirmed'
        )
        or (
          n.template_key='appointment_cancelled'
          and a.status='cancelled'
        )
        or (
          n.template_key='appointment_rescheduled'
  ...
  stale as (
    ...
    where n.status='queued'
      and n.scheduled_for <= now()
      and (
        (n.appointment_id is not null and not exists (
          select 1
          ...
          where a.id=n.appointment_id
            and (
              ...
              or (n.template_key in ('reminder_24h','reminder_1h')
                  and a.status in ('pending','confirmed')
                  and (
                    a.deposit_status <> 'awaiting'
                    or a.hold_expires_at is null
        ...
        or
        (n.waitlist_entry_id is not null and not exists (
          select 1
...
comment on function public.claim_notifications(integer)
is 'Atomically claims due notifications whose appointment/waitlist state is still valid. Obsolete queued work is skipped before it reaches the provider.';
