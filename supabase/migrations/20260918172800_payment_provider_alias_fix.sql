-- BarberOS Phase 16 patch: explicit aliases for output-variable name collisions.
...
begin
  ...
  if jsonb_typeof(coalesce(p_public_config,'{}'::jsonb))<>'object' or jsonb_typeof(v_credentials)<>'object' then
  ...
  select * into a from public.payment_accounts x
  where x.barbershop_id=p_shop and x.provider=p_provider for update;
  ...
  if not found then
    ...
    update public.payment_accounts x
  ...
  select x.id,x.provider,x.enabled,
    (x.credential_secret_id is not null and (
...
begin
  ...
  select * into pa from public.payment_accounts x
  where x.barbershop_id=a.barbershop_id and x.provider=p_provider and x.enabled=true
  for update;
  ...
  if found then
    ...
    return query select pay.id,pay.provider,pay.provider_ref,pay.amount_cents,pay.msisdn,a.hold_expires_at,true;
    return;
  end if;

  select * into pay from public.payments x
  where x.appointment_id=a.id and x.status='pending'
  ...
  if found then
    ...
    update public.payments x set status='failed',failure_code='REPLACED_BY_NEW_ATTEMPT',
  ...
  values(a.barbershop_id,null,'payment_initiated','payment',pay.id,
    jsonb_build_object('appointment_id',a.id,'provider',pay.provider::text,'amount_cents',pay.amount_cents));
...
create or replace function public.find_payment_by_provider_ref(p_provider public.payment_provider,p_provider_ref text,p_account uuid)
...
as $fn$
 ...
 where x.provider=p_provider and x.provider_ref=p_provider_ref and x.provider_account_id=p_account limit 1;
$fn$;
