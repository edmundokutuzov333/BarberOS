-- Phase 19 hardening: daily platform metrics must count appointments by creation day only.
...
begin
  ...
  select jsonb_build_object(
    ...
    'daily',coalesce((
      ...
      left join lateral (
        select
          ...
          count(*) filter(where a.status='completed' and a.completed_at is not null and (a.completed_at at time zone v_tz)::date=d.day) as completed,
          coalesce(sum(a.price_cents) filter(where a.status='completed' and a.completed_at is not null and (a.completed_at at time zone v_tz)::date=d.day),0) as revenue_cents
    ...
    'plans',coalesce((select jsonb_agg(jsonb_build_object('code',x.code,'name',x.name,'shops',x.shops,'price_cents',x.price_cents) order by x.price_cents,x.name) from (select p.code,p.name,p.price_cents,count(s.id)::bigint shops from public.plans p left join public.barbershops s on s.plan_id=p.id group by p.id,p.code,p.name,p.price_cents) x),'[]'::jsonb)
  ) into v_result;
  return v_result;
end;
$$;
