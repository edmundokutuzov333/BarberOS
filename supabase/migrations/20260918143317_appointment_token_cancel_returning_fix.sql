...
begin
  ...
  update public.appointments set status='cancelled',cancelled_at=now(),cancel_reason=v_reason where id=a.id
  ...
  if c.phone is not null then
    ...
    values(a.barbershop_id,a.id,'whatsapp','appointment_cancelled',c.phone,now(),jsonb_build_object('manage_token',a.manage_token,'starts_at',a.starts_at,'cancel_reason',v_reason));
  end if;
  if c.email is not null then
    insert into public.notifications(barbershop_id,appointment_id,channel,template_key,recipient,scheduled_for,payload)
    values(a.barbershop_id,a.id,'email','appointment_cancelled',c.email,now(),jsonb_build_object('manage_token',a.manage_token,'starts_at',a.starts_at,'cancel_reason',v_reason));
  end if;
  insert into public.audit_logs(barbershop_id,actor_id,action,entity,entity_id,diff)
  values(a.barbershop_id,null,'appointment_cancelled','appointment',a.id,jsonb_build_object('source','token','cancel_reason',v_reason,'previous_status',a.status::text));
