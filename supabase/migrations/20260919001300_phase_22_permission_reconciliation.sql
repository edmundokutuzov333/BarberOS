-- Phase 22: final permission reconciliation after the existing admin security migrations.
...
revoke insert, update, delete on public.profiles, public.barbershop_members, public.customers
from anon, authenticated;
revoke all on public.payment_accounts, public.support_tickets, public.audit_logs
from anon, authenticated;
...
grant insert, update, delete on public.barbershops, public.services, public.haircuts,
  public.barbers, public.barber_services, public.working_hours,
  public.schedule_overrides, public.time_blocks
to authenticated;
...
for update
using (id = auth.uid())
with check (id = auth.uid() and is_platform_admin = private.is_platform_admin());
...
returns table(id uuid,user_id uuid,role public.app_role,full_name text,email text,created_at timestamptz)
language plpgsql
stable
security definer
set search_path=''
as $$
begin
  if p_shop is null then raise exception 'SHOP_REQUIRED'; end if;
  if private.is_platform_admin() then
    null;
  elsif not private.is_member(p_shop,array['owner']::public.app_role[]) then
